...
if(!class_exists('th23_admin_v160')) {
	class th23_admin_v160 {
		...
		function show_screen_options($html, $screen) {
			...
			$html .= '<input type="hidden" id="th23-admin-screen-options-plugin" value="' . $this->parent->plugin['slug'] . '" />';
		...
		function set_screen_options() {
			if(!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'th23-admin-screen-options-nonce')) {
				...
				update_user_meta(get_current_user_id(), 'th23_admin_screen_options-' . sanitize_text_field($_POST['plugin']), $screen_options);
			}
			wp_die();
		}
